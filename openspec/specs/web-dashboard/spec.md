...
The web dashboard SHALL display a list of current free models.
...
- **AND** display model cards in a responsive grid layout
...
Each model SHALL be displayed as a card with key information.

#### Scenario: Model card content
- **WHEN** a model is displayed
- **THEN** the card SHALL show: model name, description, modality badge, context length, prompt price, and completion price
- **AND** the model ID SHALL be displayed in a code block at the bottom
...
Model cards SHALL be displayed in a responsive grid.
...
- **WHEN** the viewport width is >= 768px and < 1024px
